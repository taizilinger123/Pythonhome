...
#      except和finally至少有一个
#    
#      可以将可能出现的代码放入到try语句，这样如果代码没有错误，则会正常执行，如果出现错误，则会执行except子句中的代码，这样我们就可以通过
#      代码来处理异常避免因为一个异常导致整个程序的终止
# 
# 异常的传播（抛出异常）
#   当在函数中出现异常时，如果在函数中对异常进行了处理，则异常不会再继续传播，如果函数中没有对异常进行处理，则异常会继续向函数调用处传播
#   如果函数调用处处理了异常，则不再传播，如果没有处理则继续向调用处传播直到传递到全局作用域(主模块)如果依然没有处理，则程序终止，并且显示异常信息
#   
